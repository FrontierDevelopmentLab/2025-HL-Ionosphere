...
def parse_ionex_to_dataframe(file_path):
    ...
    while i < len(lines):
        if 'START OF TEC MAP' in lines[i]:
            ...
            if h == 24:
                # TODO: Double check what to do with h=24, which is a duplication of the next day h=0 mi=0. Currently we have 97 files per day, should be 96
                h = 0
...
def main():
    ...
    import random
    # files = random.sample(files, 100)  # Shuffle files to avoid any bias in processing order
