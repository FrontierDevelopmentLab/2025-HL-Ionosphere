...
class IonCastLSTMSDO(nn.Module):
    def __init__(self, input_channels=17, output_channels=17, base_channels=8, lstm_dim=1024, num_layers=2, 
        ...
        self.sdo_num_layers = sdo_num_layers

        # Original spatial processing components
        ...
        # SDO context LSTM - processes the 21504-dim vectors (same hidden size as main LSTM)
        self.sdo_lstm = nn.LSTM(input_size=sdo_dim, hidden_size=lstm_dim, 
                               num_layers=sdo_num_layers, batch_first=True, dropout=dropout if sdo_num_layers > 1 else 0)

        # Main spatial LSTM
        self.lstm = nn.LSTM(input_size=embedding_dim, hidden_size=lstm_dim, num_layers=num_layers, 
        ...
        print('  sdo_dim:', sdo_dim)

    def init_with_sdo_context(self, sdo_context):
        """Initialize the main LSTM hidden state using SDO context vectors."""
        device = next(self.parameters()).device
        sdo_context = sdo_context.to(device)
        
        # Process SDO context through context LSTM
        # sdo_context shape: (B, context_window, sdo_dim)
        sdo_output, (sdo_h, sdo_c) = self.sdo_lstm(sdo_context)
        
        # Use the final hidden state from SDO LSTM to initialize main LSTM
        # sdo_h shape: (sdo_num_layers, B, lstm_dim)
        # We need: (num_layers, B, lstm_dim)
        
        # Take the final layer's hidden state
        final_sdo_h = sdo_h[-1]  # (B, lstm_dim)
        final_sdo_c = sdo_c[-1]  # (B, lstm_dim)
        
        # Replicate across all layers of main LSTM
        main_h = final_sdo_h.unsqueeze(0).repeat(self.num_layers, 1, 1)  # (num_layers, B, lstm_dim)
        main_c = final_sdo_c.unsqueeze(0).repeat(self.num_layers, 1, 1)  # (num_layers, B, lstm_dim)
        
        self.hidden = (main_h, main_c)
    ...
    def loss(self, data, sdo_context, jpld_channel_index=0, jpld_weight=1.0):
        ...
        # Initialize LSTM state with SDO context
        self.init_with_sdo_context(sdo_context)
    ...
    def predict(self, data_context, sdo_context, prediction_window=4):
        """ Forecasts the next time steps given the context window and SDO context. """
