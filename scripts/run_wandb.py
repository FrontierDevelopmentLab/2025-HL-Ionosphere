...
def main():
    ...
    parser.add_argument('--hidden_dim', type=int, default=64, help='Hidden dimension for model architecture')
    parser.add_argument('--lstm_dim', type=int, default=1024, help='LSTM dimension for IonCastLSTM model')
    ...
    with Tee(log_file):
            ...
            if len(model_files) > 0:
                ...
                if args.model_type == 'IonCastConvLSTM':
                    ...
                    model = IonCastLSTM(input_channels=total_channels, output_channels=total_channels, 
                                      hidden_dim=args.hidden_dim, lstm_dim=args.lstm_dim, num_layers=args.num_layers,
                                      context_window=args.context_window, dropout=args.dropout)
                else:
                    raise ValueError('Unknown model type: {}'.format(args.model_type))
        ...
        if wandb is not None and args.wandb_mode != 'disabled':
            ...
            if args.mode == 'train' and len(valid_rmse_losses) > 0:
                final_valid_rmse = valid_rmse_losses[-1][1]  # Get last validation RMSE
                final_valid_jpld_rmse = valid_jpld_rmse_losses[-1][1] if valid_jpld_rmse_losses else final_valid_rmse
                final_train_loss = train_losses[-1][1] if train_losses else None
                wandb.run.summary["final_valid_rmse"] = final_valid_rmse
                wandb.run.summary["final_valid_jpld_rmse"] = final_valid_jpld_rmse
                wandb.run.summary["best_valid_rmse"] = best_valid_rmse
                wandb.run.summary["final_train_loss"] = final_train_loss

                # Also log key hyperparameters for easy filtering
                wandb.run.summary["model_type"] = args.model_type
                wandb.run.summary["learning_rate"] = args.learning_rate
                wandb.run.summary["batch_size"] = args.batch_size
                wandb.run.summary["context_window"] = args.context_window
                wandb.run.summary["prediction_window"] = args.prediction_window
                wandb.run.summary["dropout"] = args.dropout
                wandb.run.summary["hidden_dim"] = args.hidden_dim
                wandb.run.summary["lstm_dim"] = args.lstm_dim
                wandb.run.summary["num_layers"] = args.num_layers
                wandb.run.summary["jpld_weight"] = args.jpld_weight
                wandb.run.summary["delta_minutes"] = args.delta_minutes
