...
def load_model(file_name, device):
    ...
    if checkpoint['model'] == 'IonCastConvLSTM':
        ...
        input_res = checkpoint.get("input_res", FIXED_IMAGE_SIZE)  # Default to FIXED_IMAGE_SIZE if not specified
...
def main():
    ...
    parser.add_argument('--set_columns', nargs='+', default=set_all_columns, help='List of SET dataset columns to use')
    ...
    parser.add_argument('--aux_datasets', nargs='+', choices=["sunmoon", "omni", "celestrak", "set", "quasidipole"], default=[], help="additional datasets to include on top of TEC maps") # ["sunmoon", "quasidipole", "celestrak", "omni", "set"]
    ...
    dataset_constructors = { 
            ...
            'set': lambda date_start_=None, date_end_=None, date_exclusions_=None, column_=args.set_columns: SET(file_name=dataset_set_file_name, date_start=date_start_, date_end=date_end_, normalize=True, date_exclusions=date_exclusions_, delta_minutes=FIXED_CADENCE,column=column_, return_as_image_size=FIXED_IMAGE_SIZE),
    ...
    with Tee(log_file):
        ...
        if args.mode == 'train':
            ...
            if args.valid_event_id:
                ...
                # Loop over dictionary items and convert to Union datasets
                aux_datasets_valid = []
            ...
            if args.model_type == 'IonCastConvLSTM' or args.model_type == 'IonCastLSTM':
                ...
                dataset_celestrak_train = CelesTrak(dataset_celestrak_file_name, date_start=date_start, date_end=date_end, return_as_image_size=FIXED_IMAGE_SIZE)
                dataset_celestrak_valid = CelesTrak(dataset_celestrak_file_name, date_start=dataset_omniweb_valid.date_start, date_end=dataset_omniweb_valid.date_end, return_as_image_size=FIXED_IMAGE_SIZE)
                dataset_omniweb_train = OMNIWeb(dataset_omniweb_dir, date_start=date_start, date_end=date_end, column=args.omniweb_columns, return_as_image_size=FIXED_IMAGE_SIZE)
            ...
            if args.cache_dir:
                ...
                os.makedirs(valid_cache_dir, exist_ok=True) # Create if it does not exist
                        ...
                        for batch in tqdm(valid_loader, desc='Validation', leave=False):
                            if args.model_type in ['IonCastConvLSTM', 'IonCastLSTM', 'IonCastLinear']:
                                ...
                                if args.sunlock_features:
                                    ...
                                    subsolar_lats, subsolar_lons = subsolar_lats.to(device), subsolar_lons.to(device)
                                    # grid_nodes = sunlock_features(grid_nodes, subsolar_lats, subsolar_lons, image_indices=image_indices, latitude_lock=False)  
                                    grid_nodes = sunlock_features(grid_nodes, subsolar_lats, subsolar_lons, image_indices=None, latitude_lock=False)  # NOTE: setting image indices to None to see if it crashes again, this will cause all channels to be used by setting image_indices to range(n_channels)
            ...
            with torch.no_grad():
                ...
                for event_id in args.test_event_id:
                    ...
                    if args.model_type == 'IonCastConvLSTM' or args.model_type == 'IonCastLSTM':
                        dataset_jpld = JPLD(dataset_jpld_dir, date_start=buffer_start, date_end=event_end)
                        ...
                        dataset_celestrak = CelesTrak(dataset_celestrak_file_name, date_start=buffer_start, date_end=event_end, return_as_image_size=FIXED_IMAGE_SIZE)
...
# CUDA_LAUNCH_BLOCKING=1 python run_ioncast.py --data_dir /home/jupyter/data --aux_dataset sunmoon quasidipole celestrak omni set --mode train --target_dir /home/jupyter/halil_debug/ioncastgnn-fulldataset-sunlock-debug1 --num_workers 12 --batch_size 1 --model_type IonCastGNN --epochs 1000 --learning_rate 3e-4 --weight_decay 0.0 --context_window 4 --prediction_window 1 --num_evals 1 --jpld_weight 2.0 --date_start 2010-05-13T00:00:00 --date_end 2024-08-01T00:00:00 --mesh_level 4 --device cuda:0 --valid_event_id G0H3-201704230900 --save_all_models --residual_target --wandb_run_name IonCastGNN --date_dilation 64000 --wandb_run_name IonCastGNN_sunlocked --sunlock_features
...
# October 1st 00:38 UK time (Crashed reran on October 1s 11:47 UK time)
...
# 96 context sunlocked non-residual run
# CUDA_VISIBLE_DEVICES=1 python run_ioncast.py --data_dir /home/jupyter/data --aux_dataset sunmoon quasidipole celestrak omni set --mode train --target_dir /home/jupyter/halil_debug/ioncastgnn-fulldataset-sunlock-dilation64-nonresidual-Oct-5 --num_workers 12 --batch_size 1 --model_type IonCastGNN --epochs 1000 --learning_rate 3e-4 --weight_decay 0.0 --context_window 96 --prediction_window 1 --num_evals 1 --jpld_weight 2.0 --date_start 2010-05-13T00:00:00 --date_end 2024-08-01T00:00:00 --mesh_level 6 --device cuda --valid_event_id validation_events_1 --valid_every_nth_epoch 1 --save_all_models --max_valid_samples 1400 --wandb_run_name IonCastGNN_sunlocked --sunlock_features --date_dilation 64 --lead_times 180
# CUDA_VISIBLE_DEVICES=1 python run_ioncast.py --data_dir /home/jupyter/data --aux_dataset sunmoon quasidipole celestrak omni set --mode test  --target_dir /home/jupyter/halil_debug/ioncastgnn-fulldataset-sunlock-dilation64-nonresidual-Oct-5 --num_workers 12 --batch_size 1 --model_type IonCastGNN --epochs 1000 --learning_rate 3e-4 --weight_decay 0.0 --context_window 96 --prediction_window 1 --num_evals 1 --jpld_weight 2.0 --date_start 2010-05-13T00:00:00 --date_end 2024-08-01T00:00:00 --mesh_level 6 --device cuda --model_file /home/jupyter/halil_debug/ioncastgnn-fulldataset-sunlock-dilation64-nonresidual-Oct-5/epoch-06-model.pth --lead_times 15 30 60 90 120 180 360 720 --wandb_run_name IonCastGNN_sunlocked --test_event_id G0H12-201804202100 G2H12-201509071500 G4H12-202304231500 --sunlock_features
# (Reference test args)  python run_ioncast.py --data_dir /home/jupyter/data --aux_dataset sunmoon quasidipole celestrak omni set --mode test  --target_dir /home/jupyter/halil_debug/ioncastgnn-fulldataset-dilation256_Aug22_context96_test     --num_workers 12 --batch_size 1 --model_type IonCastGNN --epochs 1000 --learning_rate 3e-4 --weight_decay 0.0 --context_window 96 --prediction_window 1 --num_evals 1 --jpld_weight 2.0 --date_start 2010-05-13T00:00:00 --date_end 2024-08-01T00:00:00 --mesh_level 6 --device cuda:0 --model_file /home/jupyter/halil_debug/ioncastgnn-fulldataset-dilation256_Aug22_context96/epoch-06-model.pth.       --lead_times 60 120  --residual_target   --wandb_run_name IonCastGNN           --test_event_id G0H12-201804202100 G2H12-201509071500 G4H12-202304231500