...
class VAE(nn.Module):
    def __init__(self, z_dim=512, c_dim=0, sigma_vae=False):
        super().__init__()
        self.z_dim = z_dim
        self.c_dim = c_dim
        ...
        # test_c_input = torch.randn(1, c_dim)
        test_output = self.encoder(test_input)
        dim_after_encoder = test_output.nelement()

        self.fc1 = nn.Linear(dim_after_encoder + c_dim, z_dim)
        self.fc2 = nn.Linear(dim_after_encoder + c_dim, z_dim)
        self.fc3 = nn.Linear(z_dim + c_dim, dim_after_encoder)
    ...
    def sample(self, c, mu=None, logvar=None, n=None):
...
class IonCastConvLSTM(nn.Module):
    ...
    def __init__(self, input_channels=1, output_channels=1, hidden_dim=64, num_layers=4):
        super().__init__()
