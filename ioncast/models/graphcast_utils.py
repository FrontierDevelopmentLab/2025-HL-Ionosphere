...
def process_to_grid_nodes(
    ...
    if include_timestamp: # NOTE: double check tomorrow type of the timestamps (either datetime obj or pandas datetime)
        ...
        minute_of_day = lambda timestamp: datetimetimestamp.hour * 60 + timestamp.minute
        ...
        # current shape is [T*B,]
        ts_time_of_day = [minute_of_day(timestamp) for ts_sequence_step in timestamps for timstamp in ts_sequence_step] 
        ts_day_of_year = [day_of_year(timestamp) for ts_sequence_step in timestamps for timstamp in ts_sequence_step] 

        # current shape is [B, T] NOTE: Need to double check if flattening and reshaping back to original happening along the right dimensions
        sin_local_time_of_day = torch.sin(2 * np.pi * ts_time_of_day).reshape(T, B).T
        cos_local_time_of_day = torch.cos(2 * np.pi * ts_time_of_day).reshape(T, B).T 
        sin_year_progress = torch.sin(2 * np.pi * ts_day_of_year).reshape(T, B).T
        cos_year_progress = torch.cos(2 * np.pi * ts_day_of_year).reshape(T, B).T 

        # Create a tensor of shape B, 4*T, H, W (copy over H and W dimensions)
        time_tensor = torch.zeros(B, T, 4, H, W) # 4 channels, 
        time_tensor[:, :, 0, :, :] = sin_local_time_of_day[:, :, None, None].repeat(1, 1, H, W) # repeat until [B, T, H, W]
        time_tensor[:, :, 1, :, :] = cos_local_time_of_day[:, :, None, None].repeat(1, 1, H, W)
        time_tensor[:, :, 2, :, :] = sin_year_progress[:, :, None, None].repeat(1, 1, H, W) # 
        time_tensor[:, :, 3, :, :] = cos_year_progress[:, :, None, None].repeat(1, 1, H, W)
    ...
    if model_type == 'GraphCast_forecast':
        ...
        time_tensor = time_tensor.reshape(B, F*T) 
